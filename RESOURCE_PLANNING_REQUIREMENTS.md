...
- `ПроектID` - ID проекта, получаемый на вход как GET-параметр
- `enableCleanup` - Включить/Отключить очистку данных по планированию (по умолчанию true)
...
```
GET https://{host}/{db}/report/2681?JSON_KV&?ProjID={ПроектID}
```
...
- 'Захватка (координаты)' - название площадки для работы и её координаты
...
| Поле | Код | Описание |
|------|-----|----------|
| Длительность задачи | `t3758` | Длительность в минутах |
| Длительность операции | `t3757` | Длительность в минутах |
...
- **Захватка (координаты)** - одинаковые задачи на разных захватках могут выполняться параллельно
- Исполнитель назначается на географически ближайшую к нему захватку
