...
При запросе содержимого таблицы через метод `object` можно использовать параметры фильтрации для получения только нужных записей. Это позволяет снизить объём передаваемых данных и ускорить обработку на клиенте.

#### Базовая фильтрация

Для фильтрации по полю используется параметр `F_{ID поля}={значение}`:

```bash
curl --location 'https://integram.io/{database}/object/663?JSON_DATA&F_698=123'
```

Где:
*   `663` — ID таблицы
*   `698` — ID поля для фильтрации
*   `123` — значение для фильтрации

#### Специальные префиксы фильтрации

Для расширенной фильтрации поддерживаются специальные префиксы:

##### Префикс @ - фильтрация по ID

Префикс `@` указывает, что фильтрация должна происходить по ID записи в справочном поле, а не по значению:

```bash
curl --location 'https://integram.io/{database}/object/663?JSON_DATA&F_698=@957'
```

Этот запрос вернёт записи, где поле 698 ссылается на запись с ID 957 в справочнике.

**Пример использования:** Получить все проекты, у которых родительский проект с ID 957:

```bash
curl --location 'https://integram.io/{database}/object/663?JSON_DATA&F_698=@957'
```

##### Префикс ! - отрицание условия

Префикс `!` инвертирует условие фильтрации:

```bash
curl --location 'https://integram.io/{database}/object/663?JSON_DATA&F_698=!123'
```

Этот запрос вернёт записи, где поле 698 НЕ равно значению 123.

##### Комбинация !@ - НЕ равно ID

Префиксы можно комбинировать. Комбинация `!@` означает "НЕ равно ID":

```bash
curl --location 'https://integram.io/{database}/object/663?JSON_DATA&F_698=!@957'
```

Этот запрос вернёт записи, где поле 698 НЕ ссылается на запись с ID 957.

**Пример использования:** Получить все проекты, у которых родительский проект НЕ является "Типовой проект" (ID 957):
...
Можно применять несколько фильтров одновременно, объединяя их через `&`:

```bash
curl --location 'https://integram.io/{database}/object/663?JSON_DATA&F_698=!@957&F_700=@810'
```

Этот запрос вернёт записи, где:
*   Поле 698 НЕ ссылается на ID 957
*   Поле 700 ссылается на ID 810

#### Синтаксис префиксов

Порядок префиксов имеет значение:
*   `!` всегда идёт первым (отрицание)
*   `@` идёт вторым (фильтрация по ID)
*   Затем идёт значение

**Правильно:**
*   `F_698=123` — поле равно 123
*   `F_698=@957` — поле равно ID 957
*   `F_698=!123` — поле не равно 123
*   `F_698=!@957` — поле не равно ID 957

**Неправильно:**
*   `F_698=@!957` — неверный порядок префиксов
